...
Finally, we have uploaded an additional folder `Results.zip` to Zenodo, with DOI: . This folder contains all the simulations and chains produced, including the randomised starts, and it can be found here. This folder is not really necessary to reproduce the paper, but should the user want to use it, ideally it should be located at the structural level as the folders `Data`, `Figures` and `Scripts`.
