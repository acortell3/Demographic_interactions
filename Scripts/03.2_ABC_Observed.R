...
n_sim <- c(1:100)
for (j in 1:3){
  for (k in 1:length(n_sim)){## Load data
    Neo_spd <- readRDS(paste0("Data/Observed/",geographic_area[j],"/",geographic_area[j],"_",as.character(beginning),"_years_before_f.rds"))
    Meso_spd <- readRDS(paste0("Data/Observed/",geographic_area[j],"/",geographic_area[j],"_",as.character(beginning),"_years_before_hg.rds"))
    Sd_hg <- readRDS(paste0("Data/Observed/",geographic_area[j],"/",geographic_area[j],"_",as.character(beginning),"_years_before_Sd_hg.rds"))
    Sd_f <- readRDS(paste0("Data/Observed/",geographic_area[j],"/",geographic_area[j],"_",as.character(beginning),"_years_before_Sd_f.rds"))
    ...
    saveRDS(theta_prev,file = paste0("Results/Observed/",geographic_area[j],"/",geographic_area[j],"_",beginning,"_years_before_Parameters_fit_rej_",n_sim[k],".rds"))
    write.csv(d_theta_rej,file =  paste0("Results/Observed/",geographic_area[j],"/",geographic_area[j],"_",beginning,"_years_before_Distances_fit_",n_sim[k],".rds"))
    ...
    while (n_theta <= 6){
      ...
      if (length(theta_i)>control){
        ...
        file_dir <- paste0("Results/Observed/",geographic_area[j],"/",geographic_area[j],"_",beginning,"_years_before_",length(theta_i),"_theta_",n_theta,"_",n_sim[k],".csv")
      ...
      if (length(theta_i)>part_sim){
        ...
        saveRDS(theta_i,file = paste0("Results/Observed/",geographic_area[j],"/",geographic_area[j],"_",beginning,"_years_before_theta_part_",n_theta,"_fit_",n_sim[k],".rds"))
        saveRDS(theta_i_fit,file = paste0("Results/Observed/",geographic_area[j],"/",geographic_area[j],"_",beginning,"_years_before_theta_i_fit_part_",n_theta,"_fit_",n_sim[k],".rds"))
